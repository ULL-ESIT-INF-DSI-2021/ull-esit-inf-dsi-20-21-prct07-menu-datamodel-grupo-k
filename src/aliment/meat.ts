...
/**
 * clase que representa a la categoria de un alimento. en este caso representa a las carnes
 */
export class Meat extends Aliment {
  /**
   * atributo del tipo gupo de alimentos
   * para identificar que es un alimento de tipo Meat
   */
   ...
   */
    ...
    */
    printAliment(): string {
      const aliment_aux = new Meat(this.getName(), this.getPriceOfAliment(), this.getProtein(), this.getFats(), this.getCarbohydrates(), this.getCalories(), this.getStarch(), this.getSugars(), this.getFiber(), this.getWater(), this.getLocality(), this.getCity());
