...
/**
 * clase que representa a la categoria de un alimento. en este caso representa a los Granos.
 */
export class Seed extends Aliment {
  /**
   * atributo del tipo gupo de alimentos
   * para identificar que es un alimento de tipo Seed.
   */
    private readonly AlimentGroup: alimentGroup = alimentGroup.Seed
   ...
   */
    ...
    */
    printAliment(): string {
      const aliment_aux = new Seed(this.getName(), this.getPriceOfAliment(), this.getProtein(), this.getFats(), this.getCarbohydrates(), this.getCalories(), this.getStarch(), this.getSugars(), this.getFiber(), this.getWater(), this.getLocality(), this.getCity());
