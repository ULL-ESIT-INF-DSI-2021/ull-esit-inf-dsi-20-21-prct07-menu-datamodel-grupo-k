...
/**
 * clase que representa a la categoria de un alimento. En este caso a las legumbres.
 */
export class Legume extends Aliment {
    /**
   * atributo del tipo gupo de alimentos
   * para identificar que es un alimento de tipo legume.
   */
    private readonly AlimentGroup: alimentGroup = alimentGroup.Legume
   ...
   */
    ...
    */
    printAliment(): string {
      const aliment_aux = new Legume(this.getName(), this.getPriceOfAliment(), this.getProtein(), this.getFats(), this.getCarbohydrates(), this.getCalories(), this.getStarch(), this.getSugars(), this.getFiber(), this.getWater(), this.getLocality(), this.getCity());
